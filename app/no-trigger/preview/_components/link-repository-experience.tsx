...
const LINK_TRANSITION_DELAY_MS = 1000;
...
function deriveRepositoryLabel(repositoryFullName?: string) {
  if (!repositoryFullName) {
    return "project";
  }

  const slug = repositoryFullName.split("/").pop()?.trim() ?? "";
  ...
  const sanitized = firstToken.replace(/[^a-zA-Z0-9]/g, "").trim();
...
export function LinkRepositoryExperience() {
  ...
  const repositoryLabel = useMemo(
    () => deriveRepositoryLabel(selectedRepository?.name),
  ...
  const handleLinkRepository = () => {
    ...
    }, LINK_TRANSITION_DELAY_MS);
  };

  return (
    <main className="min-h-screen overflow-hidden bg-[#05050A] text-[#F2F1F7]">
      <div className="grid min-h-screen">
        <div
          ...
          className={`col-start-1 row-start-1 transition-[opacity,transform,filter] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            isMainScreenVisible
              ? "pointer-events-none translate-y-2 scale-[0.992] opacity-0 blur-[1.5px]"
              : "translate-y-0 scale-100 opacity-100 blur-0"
          }`}
        >
          <div className="flex min-h-screen w-full justify-center bg-[linear-gradient(180deg,#131720_0%,#0D111A_100%)] px-4 py-5 sm:px-6 sm:py-8">
            <section
              className={`w-full max-w-[576px] rounded-[22px] border border-white/8 bg-[#0A0813] px-4 py-5 shadow-[0_24px_90px_rgba(0,0,0,0.38)] transition-transform duration-300 sm:px-6 sm:py-6 ${
                isLinking ? "scale-[0.997]" : "scale-100"
              }`}
            >
              <div className="flex flex-col gap-5">
                ...
                >
                  <span className="inline-flex items-center gap-2">
                    {isLinking ? (
                      <span
                        className="size-3.5 animate-spin rounded-full border border-white/45 border-t-transparent"
                        aria-hidden="true"
                      />
                    ) : null}
