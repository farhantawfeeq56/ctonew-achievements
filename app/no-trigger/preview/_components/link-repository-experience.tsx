...
function getRepositoryLabel(fullRepositoryName: string) {
  const [, slug = fullRepositoryName] = fullRepositoryName.split("/");
  const firstToken = slug
    .split("-")
  ...
  const sanitizedToken = firstToken.replace(/[^a-zA-Z0-9]/g, "").trim();

  if (!sanitizedToken) {
    return "project";
  }

  return /[A-Z]/.test(sanitizedToken) ? sanitizedToken : sanitizedToken.toLowerCase();
}

export function LinkRepositoryExperience() {
  const [selectedRepositoryId, setSelectedRepositoryId] = useState<string | null>(null);
  const [isLinking, setIsLinking] = useState(false);
  const [isMainScreenVisible, setIsMainScreenVisible] = useState(false);
  const [linkedRepositoryLabel, setLinkedRepositoryLabel] = useState("project");
  ...
  const handleLink = () => {
    if (!selectedRepository || isLinking) {
      return;
    }

    setLinkedRepositoryLabel(getRepositoryLabel(selectedRepository.name));
    setIsLinking(true);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#05050A]">
      <div
        aria-hidden={isMainScreenVisible}
        className={`transition-[opacity,transform,filter] duration-500 ease-out ${
          isMainScreenVisible
            ? "pointer-events-none translate-y-2 scale-[0.995] opacity-0 blur-[1px]"
            ...
            <section className="w-full max-w-[576px] rounded-[22px] border border-white/8 bg-[#0A0813] px-4 py-5 shadow-[0_24px_90px_rgba(0,0,0,0.38)] sm:px-6 sm:py-6">
              <div className="flex flex-col gap-5">
                ...
                <button
                  ...
                  onClick={handleLink}
                  aria-busy={isLinking}
                  className="flex h-14 w-full items-center justify-center rounded-[12px] bg-[#3320D8] font-['Instrument_Sans',system-ui,sans-serif] text-[16px] leading-6 font-medium tracking-[-0.01em] text-white/85 transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8ca6ff] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0A0813] enabled:hover:bg-[#3b27ec] enabled:active:scale-[0.995] disabled:cursor-not-allowed disabled:bg-[#2A2732] disabled:text-[#8E8A99]"
                >
                  {isLinking ? (
                    <span className="inline-flex items-center gap-2.5">
                      <span
                        className="size-3.5 animate-spin rounded-full border-2 border-white/35 border-t-white"
                        aria-hidden="true"
                      />
                      <span>Linking repository...</span>
                    </span>
                  ) : (
                    "Link"
                  )}
      ...
      <div
        ...
        className={`absolute inset-0 transition-[opacity,transform] duration-500 ease-out ${
          isMainScreenVisible ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-2 opacity-0"
        }`}
      >
        <MainScreenNew repositoryLabel={linkedRepositoryLabel} />
      </div>
    </div>
  );
}
